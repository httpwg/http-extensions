...
A server that supports the creation of a resumable upload resource ({{upload-creation}}) for a target URI MUST include the following header fields in response to an `OPTIONS` request for the target URI: `Upload-Limit` with the corresponding limits, and `Accept-Patch` ({{Section 3.1 of PATCH}}) with the value of `application/partial-upload`. If a server supports the creation of upload resources for any target URI, it SHOULD include the following header fields in response to an `OPTIONS` request for the target `*` (if this target is supported): `Upload-Limit` with the corresponding limits, and `Accept-Patch` with the value of `application/partial-upload`. If the server does not apply any limits, it MUST use `min-size=0` instead of an empty header value.
