...
- Final responses for appending representation data ({{upload-appending}}) with a `2XX` status code and the `Upload-Complete: ?0` header acknowledge that the entire request content was received and the offset increased by the length of the appended representation data.
