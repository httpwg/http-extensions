...
* Each endpoint MUST close its send stream when the corresponding TCP connection reaches the TIME-WAIT or CLOSED state.
...
* The endpoint SHOULD send a TCP RST when the receive stream is terminated if the TCP subsystem permits it and
  - the receive stream ended abruptly, or
  - a FINAL_DATA capsule was not received.
