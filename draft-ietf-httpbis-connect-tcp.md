...
An END_DATA capsule indicates that sender has finished sending data for this stream.  Its payload is a QUIC varint representing an error code.  A value of 0 means "completed", semantically equivalent to TCP FIN.  Any other value indicates truncated data, corresponding to a TCP RST, timeout, or other error.  In this specification, the only other defined value is 1, meaning "truncated".

A STOP_SENDING_DATA capsule indicates that the sender has begun discarding DATA capsules that it receives on this stream.  Its payload is empty.

See {{closing-connections}} for related requirements.
...
When using "connect-tcp", connection closing is subject to the following requirements:

For all endpoints:

* If an endpoint determines that the data is complete, it MUST send an END_DATA capsule with error code 0 ("completed").
* If an endpoint determines that the data may have been truncated, it MUST send an END_DATA capsule with error code 1 ("truncated").
* An endpoint that closes its data stream before receiving an END_DATA capsule MUST send a STOP_SENDING_DATA capsule or a stream error.
* Upon receipt of a STOP_SENDING_DATA capsule or a stream error, an endpoint SHOULD NOT send any more DATA capsules on this data stream, as they will be discarded.
* An endpoint MUST NOT send STOP_SENDING_DATA after receiving an END_DATA capsule.

For endpoints translating between TCP and "connect-tcp":

* The translator MUST send a TCP FIN if it receives an END_DATA capsule with error code 0 while the TCP connection is open.
* The translator SHOULD send a TCP RST if the underlying TCP implementation permits it and
  - the translator receives a STOP_SENDING_DATA capsule, or
  - the translator receives an END_DATA capsule with any nonzero error code, or
  - the data stream is terminated without an END_DATA capsule.
* If a translator receives an indication that the TCP endpoint is discarding outbound data (e.g. TCP RST), it MUST send a STOP_SENDING_DATA capsule in addition to the END_DATA capsule.
...
| (TBD) | END_DATA          | permanent | (This document), {{specification}} | IETF              | HTTPBIS |
| (TBD) | STOP_SENDING_DATA | permanent | (This document), {{specification}} | IETF              | HTTPBIS |

For this draft version of the protocol, the Capsule Type values `0x2028d7ee`, `0x2028d7f0`, and `0x2028d7f1` shall be used provisionally for testing, under the names "DATA-07", "END_DATA-07", and "STOP_SENDING_DATA-07".
