...
When using the Capsule Protocol, TCP data is sent using a new Capsule Type named DATA ({{data-capsule}}).  When used with the "connect-tcp" Upgrade Token, a DATA capsule conveys ordinary TCP payload data.  Future specifications may establish its meaning when used with other Upgrade Tokens.
