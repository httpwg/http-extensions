...
names := http2-encryption alt-svc tunnel-protocol auth-info cice
...
friendly_names := encryption alt-svc tunnel-protocol auth-info cice
...
tunnel-protocol.%: draft-ietf-httpbis-tunnel-protocol.%
	cp -f $< $@
