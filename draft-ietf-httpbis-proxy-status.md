...
Proxy-Status MAY be sent in HTTP trailers, but -- as with all trailers -- it might be silently discarded along the path to the user agent, so this SHOULD NOT be done unless it is not possible to send it in headers. Additionally, an intermediary that might send Proxy-Status in a trailer section of a message that already contains a Proxy-Status header field SHOULD append a member to that header field, so that the trailer value's ordering relative to other intermediaries can be understood.

For example, if an intermediary is streaming a response and the upstream connection suddenly terminates, Proxy-Status can be appended to the trailers of the outgoing message (since the headers have already been sent).
...
The `error` parameter's value is a sf-token that is a Proxy Error Type. When present, it indicates that the proxy encountered an issue when obtaining a response.
...
* Extra Parameters:
  - alert-message: a sf-token containing the applicable description string from the TLS Alerts registry.
* Recommended HTTP status code: 502
...
* Extra Parameters:
  - status-code: a sf-integer containing the generated status code.
  - status-phrase: a sf-string containing the generated status phrase.
* Recommended HTTP status code: The applicable 4xx status code

This type helps distinguish between responses generated by intermediaries from those generated by the origin, and always indicates that the response was generated by the intermediary.
...
This type always indicates that the response was generated by the intermediary.
...
* Extra Parameters:
  - body-size: a sf-integer indicating how large the body received was. Note that it may not have been complete; i.e., the intermediary may have discarded or refused additional data.
* Recommended HTTP status code: 502
