...
The `error` parameter's value is an sf-token that is a Proxy Error Type. When present, it indicates that the intermediary encountered an issue when obtaining a response.
...
Proxy-Status: revproxy34.example.net; error=http_request_error, ExampleCDN
~~~
...
The `next-encryption` parameter's value indicates the version (as a float, not as encoded in TLS records) of TLS {{TLS}} used by the intermediary to connect to the next hop.

The value MUST be a sf-float, representing the TLS version used. If TLS was not used,
this parameter MUST be omitted.

For example:

~~~ http-message
Proxy-Status: ExampleCDN; next-tls=1.3
~~~
...
* Extra Parameters:
  - alert-id: an sf-integer containing the applicable value from the TLS Alerts registry. See {TLS}}.
  - alert-message: an sf-token or sf-string containing the applicable description string from the TLS Alerts registry. See {{TLS}}.
* Recommended HTTP status code: 502
* Only generated by intermediaries: false
* Reference: \[this document\]
