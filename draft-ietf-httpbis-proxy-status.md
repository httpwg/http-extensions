...
When adding a value to the Proxy-Status field, intermediaries SHOULD preserve the existing members of the field to allow debugging of the entire chain of intermediaries handling the request, unless explicitly configured to remove them (e.g., to prevent internal network details to leak; see {{security}}).
...
Unless a Proxy Error Type specifies otherwise, the presences of error often, but not always, indicates that response was generated by the proxy, not the origin server or any other inbound server. For example, a proxy might attempt to correct an error, or part of a response might be forwarded before the error is encountered.
...
* Extra Parameters:
  ...
  - alert-message: an sf-token containing the applicable description string from the TLS Alerts registry. See {{!RFC8446}}.
* Recommended HTTP status code: 502
* Only generated by intermediaries: false
* Reference: \[this document\]
