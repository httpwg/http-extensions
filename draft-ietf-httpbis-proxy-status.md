...
This specification uses terminology from Structured Fields {{STRUCTURED-FIELDS}} to specify syntax and parsing.
...
It is a List ({{STRUCTURED-FIELDS, Section 3.1}}). Each member of the List represents an intermediary that has handled the response. The first member represents the intermediary closest to the origin server, and the last member represents the intermediary closest to the user agent.
...
Each member of the List identifies the intermediary that inserted the value and MUST have a type of either String ({{Section 3.3.3 of STRUCTURED-FIELDS}}) or Token ({{Section 3.3.4 of STRUCTURED-FIELDS}}). Depending on the deployment, this might be a service name (but not a software or hardware product name; e.g., "ExampleCDN" is appropriate, but "ExampleProxy" is not because it doesn't identify the deployment), a hostname ("proxy-3.example.com"), an IP address, or a generated string.
...
The `error` parameter's value is an Token ({{Section 3.3.4 of STRUCTURED-FIELDS}}) that is a proxy error type. When present, it indicates that the intermediary encountered an issue when obtaining this response.
...
The `next-hop` parameter's value is a String ({{Section 3.3.3 of STRUCTURED-FIELDS}}) or Token ({{Section 3.3.4 of STRUCTURED-FIELDS}}) that identifies the intermediary or origin server selected (and used, if contacted) to obtain this response. It might be a hostname, IP address, or alias.
...
The value MUST be either a Token ({{Section 3.3.4 of STRUCTURED-FIELDS}}) or Byte Sequence ({{Section 3.3.5 of STRUCTURED-FIELDS}}), representing a TLS ALPN Protocol ID (see [](https://www.iana.org/assignments/tls-extensiontype-values/tls-extensiontype-values.xhtml#alpn-protocol-ids){:brackets="angle"}). If the protocol identifier is able to be expressed as a Token using ASCII encoding, that form MUST be used.
...
The value MUST be an Integer ({{Section 3.3.1 of STRUCTURED-FIELDS}}).
...
The `details` parameter's value is a String ({{Section 3.3.3 of STRUCTURED-FIELDS}}) containing additional information not captured anywhere else. This can include implementation-specific or deployment-specific information.
...
Extra Parameters:
: rcode:
  : An String ({{Section 3.3.3 of STRUCTURED-FIELDS}}) conveying the DNS RCODE that indicates the error type. See {{RFC8499, Section 3}}.

  info-code:
  : An Integer ({{Section 3.3.1 of STRUCTURED-FIELDS}}) conveying the Extended DNS Error Code INFO-CODE. See {{!RFC8914}}.
...
Extra Parameters:
: alert-id:
  : An Integer ({{Section 3.3.1 of STRUCTURED-FIELDS}}) containing the applicable value from the "TLS Alerts" registry. See {{!RFC8446}}.

  alert-message:
  : A Token ({{Section 3.3.4 of STRUCTURED-FIELDS}}) or String ({{Section 3.3.3 of STRUCTURED-FIELDS}}) containing the applicable description string from the TLS Alerts registry. See {{!RFC8446}}.
...
Extra Parameters:
: status-code:
  : An Integer ({{Section 3.3.1 of STRUCTURED-FIELDS}}) containing the generated status code.

  status-phrase:
  : A String ({{Section 3.3.3 of STRUCTURED-FIELDS}}) containing the generated status phrase.
...
Extra Parameters:
: header-section-size:
  : An Integer ({{Section 3.3.1 of STRUCTURED-FIELDS}}) indicating how large the received headers were. Note that they might not be complete; i.e., the intermediary may have discarded or refused additional data.
...
Extra Parameters:
: header-name:
  : A String ({{Section 3.3.3 of STRUCTURED-FIELDS}}) indicating the name of the header field that triggered the error.

  header-size:
  : An Integer ({{Section 3.3.1 of STRUCTURED-FIELDS}}) indicating the size of the header field that triggered the error.
...
Extra Parameters:
: body-size:
  : An Integer ({{Section 3.3.1 of STRUCTURED-FIELDS}}) indicating how large the received body was. Note that it may not have been complete; i.e., the intermediary may have discarded or refused additional data.
...
: trailer-section-size:
  : An Integer ({{Section 3.3.1 of STRUCTURED-FIELDS}}) indicating how large the received trailers were. Note that they might not be complete; i.e., the intermediary may have discarded or refused additional data.
...
Extra Parameters:
: trailer-name:
  : A String ({{Section 3.3.3 of STRUCTURED-FIELDS}}) indicating the name of the trailer field that triggered the error.

  trailer-size:
  : An Integer ({{Section 3.3.1 of STRUCTURED-FIELDS}}) indicating the size of the trailer field that triggered the error.
...
Extra Parameters:
: coding:
  : A Token ({{Section 3.3.4 of STRUCTURED-FIELDS}}) containing the specific coding (from the "HTTP Transfer Coding Registry") that caused the error.
...
Extra Parameters:
: coding:
  : A Token ({{Section 3.3.4 of STRUCTURED-FIELDS}}) containing the specific coding (from the "HTTP Content Coding Registry") that caused the error.
...
Name:
: \[a name for the proxy error type that is of type Token ({{Section 3.3.4 of STRUCTURED-FIELDS}})\]
...
IANA has created the "HTTP Proxy-Status Parameters" registry and the "HTTP Proxy Error Types" registry at [](https://www.iana.org/assignments/http-proxy-status){:brackets="angle"} and has populated them with the types defined in Sections {{<params}} and {{<error-types}} respectively; see Sections {{<register-param}} and {{<register-error}} for their associated procedures.
