...
As discussed in [common signature pitfalls](#security-signature-pitfalls), there is typically not a unique signature for a message. An attacker who has witnessed a valid signature may be able to produce multiple signatures with distinct binary representations which are still valid for the original message and public key. This means that applications SHOULD NOT rely on the binary representation of a signature for replay detection. Instead, applications SHOULD use the `nonce` signature parameter, timestamp or other appropriate message field to detect replays.
...
### Common Signature Pitfalls {#security-signature-pitfalls}

Signature schemes do not necessarily provide the properties that a designer might intuitively expect. For example, many commonly used signature schemes allow an attacker:

 - To produce a signature is valid for multiple distinct messages, or for message not known to the attacker, or even any message.

 - To produce their own public key such that a signature generated by a well-behaved party is also valid for the attacker's public key.

 - To produce their own public key such that a signature generated by a well-behaved party will also valid for the attacker's public key under a different message.

 - To take a signature produced by a well-behaved party and alter it such that it has a different binary representation but remains valid for the well-behaved party's public key and message.

These behaviours do not compromise the basic security guarentee of a signature scheme, which is that is effectively impossible to generate a signature on a new message for well-behaved party's public key without access to their private key. However, these behaviours may be surprising to designers and have been used to attack numerous protocols. A further discussion of these properties and an analysis of how they can be used to attack protocols can be found in {{JACKSON2019}}.

### Signing Signature Values {#security-sign-signature}

The properties in [common signature pitfalls](#security-signature-pitfalls) apply in particular when produce signatures of signatures.
